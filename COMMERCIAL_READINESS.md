...
- [ ] 채널 어댑터 계약 실적용 — Callbot·챗봇·D-ARS가 Core를 실제로 소비하도록 연결
      ...
      · Core 측 완료(20): **파기 계획과 저장소 사이의 빈 자리(2026-09-22)** — (13)~(19)가 메운 것과 같은 모양의
        공백이 §8.2 에도 있었다. `core/retention.ts` 에는 분류·정책 검증·만료 판정·계획 산출이 다 있는데,
        머리말이 "실제 삭제 실행은 별도 워커가 맡는다"고 적어 둔 **그 워커가 어디에도 없다** —
        `planDisposition` 을 부르는 코드는 저장소 전체에서 테스트뿐이다. 그대로 두면 이벤트·녹취·색인·
        감사로그를 든 저장소들이 각자 15줄씩 채우고 **각자 다르게 틀린다**. 그리고 여기서 틀리면 증상이
        예외가 아니라 **되돌릴 수 없는 조용한 오답**이다 — 지우면 안 되는 것을 지우거나, 지우지 않았는데
        지웠다고 적는다. 두 자리를 각각 한 곳으로 모았다.
        - 실행(`src/core/executeDisposition.ts`): 제일 비싼 함정이 **`plan.decisions` 순회**다.
          `DispositionPlan` 은 `decisions`(전체)와 `due`(대상)를 **둘 다** 들고 있고 둘 다
          `RetentionDecision[]` 이라 어느 쪽을 돌려도 타입이 통과한다 — 전체를 돌면 보존기간 한복판인
          레코드와 **법적 보류 건**까지 지우고, 어디서도 안 터지며 증상은 몇 달 뒤 "분쟁 건 녹취가 없다"로
          나타난다(executeHandoff 의 `queueId`/`admittedQueueId` 와 같은 모양이다). 그래서 **계획 객체를
          받고** `due` 에 실린 건이 정말 `status === 'due'` 인지 한 번 더 확인한다. **활성화 기본 OFF** —
          `activation: 'enabled'` + `approvalRef` 가 둘 다 있어야 포트를 부르고, 그 전에는 `dry_run` 이다.
          "드라이런인데 한 건만 실제로 나갔다"를 막으려고 **포트 호출 지점을 코드에 한 곳만** 두고 검사가
          그 사실을 고정한다 **[승인 필요]**. `anonymize`·`archive` 를 `delete` 로 대신하지 않는다(동의 이력을
          지우면 동의받았다는 증거가, 감사로그를 지우면 조사 근거가 사라진다) — 포트는 방식별로 메서드가
          갈려 있고 없는 방식은 `unsupported` 로 **드러낼 뿐 대신 처리하지 않는다**. **`{ ok: true }` 정확히
          그 모양만 파기로 적는다**: 구현을 깜빡한 스텁이 `undefined` 를 돌려줬을 때 성공으로 읽으면
          `disposedAt` 이 찍혀 다음 스윕에서 영영 제외되고 **개인정보는 남은 채 장부에만 파기로 남는다**
          (`recoveryProbe` 가 규약 위반 반환값을 `up` 으로도 `down` 으로도 적지 않은 것과 같다). 격리 위반
          말고는 **던지지 않는다** — 저장소 하나가 죽었다고 나머지 만 건이 다음 날로 밀리면 안 되고, 실패 건은
          `disposed` 가 아니므로 다음 스윕이 다시 본다. 제한 시간은 주입이며 **기본값 없음**(§13-3)이고 초과 건은
          '결과 미확인'이라 성공으로도 파기로도 적지 않는다(늦게 성공했을 수 있다). 같은 레코드는 id 기준
          한 번만 부르고, 상한 밖 건은 조용히 사라지지 않고 `not_attempted` 로 남으며, 모든 문구는 `maskPii` 를
          지난다(저장소 오류 메시지에는 지우려던 행이 그대로 들어 있다, §10.3). `blocked`·`held` 는 건수로
          드러낸다 — "0건 실패"와 "전부 막혀서 한 건도 시도 못 했다"는 다르다. 감사는 **결과를 안 뒤에**
          시도 1건당 1레코드이고(의도만 먼저 적으면 실패 건이 "지웠다"로 남아 조사에서 정반대를 가리킨다)
          드라이런은 `denied` 로 적는다. 만료·보류·승인 판정은 `retention.ts` 하나에만 있고 여기서 복사하지
          않는다(§2 — 검사가 `retentionDays`·`DAY_MS` 부재를 고정한다).
        - 저장 가능 판정(`src/core/retention.ts` `canStore`·`assertStorable`): `DataClassSpec.mayContainPii`
          주석은 "보존기간 미설정 시 **저장 자체를 막는다**"고 적어 뒀는데 그걸 막을 함수가 없었다.
          `validateRetentionPolicy` 는 **오류 문자열 목록**이라 설정 화면은 읽어도 저장하는 코드는 물어볼 데가
          없고, 문자열을 부분 일치로 뒤지는 코드가 생기면 문구를 다듬는 순간 조용히 통과한다. 막는 사고는
          **보존기간을 정하지 않은 채 개인정보가 먼저 쌓이는 것**이다 — 한 번 쌓이면 기산 시점을 되돌릴 수
          없고 정책이 없으니 `decide` 는 영원히 `blocked` 를 내, 지울 근거도 기한도 없는 개인정보가 남는다.
          저장 판정은 파기 판정과 **같은 조건**이다(갈리면 저장은 되는데 파기는 안 되는 조합이 생긴다).
          개인정보를 담지 않는 분류는 규칙이 없어도 막지 않고 경고만 낸다 — 막으면 설정을 다 채우기 전에는
          이벤트 한 건도 못 쌓아 도입 자체가 불가능해진다.
        실 DB·오브젝트 스토리지·벡터 색인 연결과 스윕 스케줄링은 **[승인 필요]**.
        근거: `src/core/executeDisposition.ts` · `src/core/retention.ts`(`canStore`·`assertStorable`) ·
        `tests/core.executeDisposition.test.mjs`(29건 — 정상·거절·예외·제한시간 초과·규약 위반 반환값·
        빈 계획·타 테넌트 포함) · `API.md` 반영
